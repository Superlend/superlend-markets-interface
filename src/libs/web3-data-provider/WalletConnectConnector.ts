...
export class PendingRequestError extends Error {
  public constructor() {
    ...
    this.name = this.constructor.name;
    this.message =
      'Request of type wallet_requestPermissions already pending for origin https://markets.superlend.xyz. Please wait.';
  }
}
...
export class WalletConnectConnector extends AbstractConnector {
  ...
  constructor() {
    ...
    const [mainnet] = supportedChainIds;
    // note: workaround for wallet connect not working correctly with empty list of optionalChains
    let optionalChains: number[] | undefined = supportedChainIds.slice(1);
    if (optionalChains.length === 0) {
      optionalChains = undefined;
    }
